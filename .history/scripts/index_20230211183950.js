...
function calcTemp(){ 
    // Cálculo de celsius para fahrenheit 
       ...
       // Kelvin para Fahrenheit
        ...
        let graus = document.getElementById('graus')
        graus = Number(graus.value)
        const resulti = (graus - 273.15) * (9/5) + 32;

        if(Number.isInteger(resulti) == true){
        res.innerHTML = `${graus} Fahrenheit em Kelvin é ${resulti.toFixed(0)} `
    } 
        else if(Number.isInteger(resulti) == false){
            res.innerHTML = `${graus} Fahrenheit em Kelvin é ${resulti.toPrecision(4)} `
        }
       } else if(unit.checked[2] && resUnit[1].checked){
        let graus = document.getElementById('graus')
        graus = Number(graus.value)
        let result = (graus - 32) * 5/9 + 273.15;
        res.innerHTML = `${graus} Fahrenheit em Kelvin é ${res}`
       }
